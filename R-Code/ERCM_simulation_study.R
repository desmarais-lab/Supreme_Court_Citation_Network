...
gamma_m<- array(0,dim=c(m,3,100) ,dimnames = list(row.names,column.names,matrix.names))
...
for(j in 1:m){
  ...
  N<- N[1:i, 1:i]
  gamma_m[j,1,i]<- sum(N)
  gamma_m[j,2,i]<- count.citation.outstars.sim(N,age=age[1:i],0.025)
  gamma_m[j,3,i]<- count.citation.triangles.sim(N,age=age[1:i],0.025)
}
}
...
gamma_N<- matrix(0, 100, 3)
colnames(gamma_N)<- c("edges", "outstars", "triangles")
rownames(gamma_N)<- 1:100

for(i in 2:100){
A.obs.cut<- A.obs[1:i, 1:i]  
gamma_N[i,1]<- sum(A.obs.cut)
gamma_N[i,2]<- count.citation.outstars.sim(A.obs.cut,age=age[1:i],0.025)
gamma_N[i,3]<- count.citation.triangles.sim(A.obs.cut,age=age[1:i],0.025)
...
ercm_iter<- function(theta, gamma_m, gamma_N, theta_0){
  ...
  ll<- 0
  for (i in 2:100){
  ll<- ll +sum(c(theta[1],theta[2],theta[3])*gamma_N[i,])- log(sum(exp(gamma_m[,,i]%*%(c(theta[1],theta[2],theta[3])- theta_0))))
  }
  -ll
}
