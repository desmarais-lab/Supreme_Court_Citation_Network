...
##################################################
library(readr)
citations.old <- read_csv("cl_citations.csv")
citations.old<- as.data.frame(citations)

# 10/4/2019
citations<- read_csv("cl_citations2.csv")
citations<- as.data.frame(citations2)
...
filenames_cluster <- list.files("all_cluster/scotus", pattern="*.json", full.names=TRUE) # data is not on git. this should give you a character vector, with each file name represented by an entry
# takes about 30 mins
...
for(i in 1:l){ # takes about 1.5 hrs
  #print(i)
...
for(i in 1: dim(overruled)[1]){
  ...
  if(sum(sender)>0 & sum(receiver)>0){ # if both are not integer(0)
...
#############################################################################
## same issue matrix, 1  same issue, 0 not
#############################################################################


same.issue<- matrix(0, d, d)
for(i in 1:d){ # calculation takes a while, ca 15 mins
  if(i %% 500 == 0) cat("Starting iteration", i, "\n")
  # issue area for sender
  issue.sender <- scc1[i, 40]
  for(j in i:d){
    # issue area for receiver
    issue.receiver <- scc1[j, 40]
    # if sender and receiver issue area is the same and non is NA then enter 1 into matrix to indicate same issue area
    if(is.na(issue.receiver)==FALSE & is.na(issue.sender)==FALSE & issue.sender==issue.receiver){
      same.issue[i,j]<-1
      same.issue[j,i]<-1
    }
  }
...
###############################################################################################
##
##  subnetwork Criminal Procedure
##
###############################################################################################

# delete all observations that are not CP
scc1.cp<- scc1
...
for(i in 10020:1){
  ...
  if(scc1.cp[i, 41]!=1 || is.na(scc1.cp[i, 41])){
    scc1.cp<- scc1.cp[-i,]
  }
}
...
adjacency.matrix.cp <- adjacency.matrix
mq.matrix.cp <- mq.matrix
same.opinion.writer.cp <- same.opinion.writer
year.diff.matrix.cp<- year.diff.matrix
...
for(i in 10020:1){
  ...
  if(scc1[i, 41]!=1 || is.na(scc1[i, 41])){
    adjacency.matrix.cp<- adjacency.matrix.cp[-i,]
    adjacency.matrix.cp<- adjacency.matrix.cp[,-i]
    mq.matrix.cp<- mq.matrix.cp[-i,]
    mq.matrix.cp<- mq.matrix.cp[,-i]
    same.opinion.writer.cp<- same.opinion.writer.cp[-i,]
    same.opinion.writer.cp<- same.opinion.writer.cp[,-i]
    year.diff.matrix.cp<- year.diff.matrix.cp[-i,]
    year.diff.matrix.cp<- year.diff.matrix.cp[,-i]
  }
}
...
## ECONOMICS includes "Unions" (7) and "Economics;" (8)

# delete all observations that are not ec
scc1.ec<- scc1
...
for(i in 10020:1){
  ...
  if(scc1.ec[i, 41]!=7  && scc1.ec[i,41]!=8|| is.na(scc1.ec[i, 41])){
    scc1.ec<- scc1.ec[-i,]
  }
}
...
adjacency.matrix.ec <- adjacency.matrix
mq.matrix.ec <- mq.matrix
same.opinion.writer.ec <- same.opinion.writer
year.diff.matrix.ec<- year.diff.matrix
...
for(i in 10020:1){
  ...
  if(scc1[i, 41]!=7 && scc1[i,41]!=8 || is.na(scc1[i, 41])){
    adjacency.matrix.ec<- adjacency.matrix.ec[-i,]
    adjacency.matrix.ec<- adjacency.matrix.ec[,-i]
    mq.matrix.ec<- mq.matrix.ec[-i,]
    mq.matrix.ec<- mq.matrix.ec[,-i]
    same.opinion.writer.ec<- same.opinion.writer.ec[-i,]
    same.opinion.writer.ec<- same.opinion.writer.ec[,-i]
    year.diff.matrix.ec<- year.diff.matrix.ec[-i,]
    year.diff.matrix.ec<- year.diff.matrix.ec[,-i]
  }
}
...
## CIVIL RIGHTS AND LIBERTIES includes the data base categories "Civil Rights," (2) "First Amendment," (3) "Due Process,' (4) and "Privacy;" (5)

# delete all observations that are not cr
scc1.cr<- scc1
...
for(i in 10020:1){
  ...
  if(scc1.cr[i, 41]!=2  && scc1.cr[i,41]!=3 && scc1.cr[i, 41]!=4  && scc1.cr[i,41]!=5 || is.na(scc1.cr[i, 41])){
    scc1.cr<- scc1.cr[-i,]
  }
}
...
adjacency.matrix.cr <- adjacency.matrix
mq.matrix.cr <- mq.matrix
same.opinion.writer.cr <- same.opinion.writer
year.diff.matrix.cr<- year.diff.matrix
...
for(i in 10020:1){
  ...
  if(scc1[i, 41]!=2 && scc1[i,41]!=3 && scc1[i, 41]!=4 && scc1[i,41]!=5 || is.na(scc1[i, 41])){
    adjacency.matrix.cr<- adjacency.matrix.cr[-i,]
    adjacency.matrix.cr<- adjacency.matrix.cr[,-i]
    mq.matrix.cr<- mq.matrix.cr[-i,]
    mq.matrix.cr<- mq.matrix.cr[,-i]
    same.opinion.writer.cr<- same.opinion.writer.cr[-i,]
    same.opinion.writer.cr<- same.opinion.writer.cr[,-i]
    year.diff.matrix.cr<- year.diff.matrix.cr[-i,]
    year.diff.matrix.cr<- year.diff.matrix.cr[,-i]
  }
}
