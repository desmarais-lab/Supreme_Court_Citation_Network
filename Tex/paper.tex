...
Let $c_{ij} \in \{0,1\}$ be a binary indicator of whether case $i$ cites $j$. Furthermore, let
$$\mathcal{C}_t=\{c(t)\in \{0,1\}^{N_t}: c_{ij}(t)\in \{0,1\} \}$$ 
be the set of all possible citation combinations for opinions issued in term $t$, where $c_{ij}(t)$ is a binary indicator of whether case $i$ cites case $j$, and case $i$ is decided at time $t$. Note that the cardinality of $\mathcal{C}_t$ increases exponentially for every newly added case.\\ The probability function of the c-TERGM  is defined as
\begin{equation}
P_{\theta}(c(t)~|~c(t-))=\cfrac{\exp\big(\theta^T\cdot h\big(c(t)~|~c(t-)\big)\big)}{\sum_{z(t)^*\in \mathcal{C}}\exp\big(\theta^T\cdot h\big(z(t)~|~c(t-)\big)\big)}
\label{ercm}
\end{equation}
where $c(t-)$ is a matrix that indicates the citations among cases decided before time $t$, $\theta \in \mathbb{R}^q$ is a $q$-dimensional vector of parameters,  $h: \mathcal{C}_t \to \mathbb{R}^q ~, ~ \c(t) \to (h_1(c(t)), \dots , h_q(c(t)))^T$ is a q-dimensional vector of different statistics and $\kappa(\theta) := \sum_{z(t)^*\in \mathcal{C}}\exp\big(\theta^T\cdot h\big(z(t)~|~c(t-)\big))$ is a normalization constant that ensures that Equation (\ref{ercm}) defines a probability function on $\mathcal{C}_t$.
...
$$h_{edges}:\mathcal{C}_t\to \mathbb{R}~~~, ~~~c(t) \to \sum_{i(t)} c_{ij}, $$
the number of citations made at time $t$, where $i(t)$ is the set of opinions issued at time $t$.  $h_{edges}$ performs the function of an intercept, and models the expected value of any given edge.
$$h_{outstar}:\mathcal{C}_t \to \mathbb{R}~~~, ~~~c(t) \to \sum_{i(t)} {\sum_{j \neq i} c_{ij} \choose 2} $$
the number of out-two-stars occuring at time $t$. An out-two-star is a configuration in which one node sends to two other nodes. The number of these configurations grows quadratically as the origins of ties concentrate on a few highly active/social senders. The out-two-stars configuration is commonly used to model sociality in ERGMs.
$$h_{instar}:\mathcal{C}_t \to \mathbb{R}~~~, ~~~c(t) \to \sum_{j(t)} {\sum_{i \neq j} c_{ij} \choose 2} $$
the number of in-two-stars occuring at time $t$. An in-two-star is a configuration in which one node receives ties from two other nodes. The number of these configurations grows quadratically as the destinations of ties concentrate on a few highly popular recipients. The out-two-stars configuration is commonly used to model popularity in ERGMs. [HERE]
